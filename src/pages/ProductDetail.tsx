...
export default function ProductDetail() {
  ...
  function handleAddToCart() {
    const productIdNumber = apiProduct
      ? (() => {
          const numId = Number(apiProduct.productId);
          return Number.isFinite(numId) ? numId : product.id;
        })()
      : product.id;

    const cartItem: Omit<CartItem, "quantity"> = apiProduct
      ? {
          id: productIdNumber,
          name: apiProduct.name,
          price: apiProduct.basePrice,
          image: apiProduct.thumbnailUrl || product.image,
        }
      : {
          id: product.id,
          name: product.name,
    ...
    toast.className =
      "fixed top-20 right-4 bg-green-600 text-white px-6 py-3 rounded-lg shadow-xl z-50";
